...
def get_varlen_pooling_list(embedding_dict, features, feature_index, device):
    varlen_sparse_embedding_list = []
    for feat in feature_index.keys():
        seq_emb = embedding_dict[feat]
...
def varlen_embedding_lookup(X, embedding_dict, sequence_input_dict):
    varlen_embedding_vec_dict = {}
    for feature_name in sequence_input_dict.keys():
        lookup_idx = sequence_input_dict[feature_name]
        if feature_name.endswith('id'):
            varlen_embedding_vec_dict[feature_name] = embedding_dict['video_id'](X[:, lookup_idx[0]:lookup_idx[1]+1].long())  # (lookup_idx)
        else:
